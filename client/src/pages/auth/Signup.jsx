...
function Signup () {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!fullName || !email || !password) return alert('Please fill all the fields')
      ...
      <div className="w-[90%] h-[90%] lg:p-12 lg:w-[65%] lg:h-full bg-white rounded-[45px] drop-shadow-2xl lg:drop-shadow-none lg:rounded-l-[55px] lg:rounded-r-none">
        <div className="w-full h-[35%] p-8 md:p-14">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <SkeltonButtonUrl btnText={'Sign up with google'} btnIcon={FacebookIcon} />
            <SkeltonButtonUrl btnText={'Sign up with Facebook'} btnIcon={GoogleIcon} />
          </div>
          <p className="hidden md:block font-montserrat text-2xl text-black font-normal my-8 md:my-12 text-center">- OR -</p>
        </div>
        <div className="w-full h-[65%]">
          <div className="flex flex-col items-center">
            <form className="flex flex-col w-[90%] mt-16 mb-4" type='action' onSubmit={(e) => handleSubmit(e)}>
              <input className="active:border-b-light-green hover:border-b-light-green focus:border-b-light-green" type="text" name="fullName" value={fullName} onChange={(e) => handleFullName(e)} placeholder={'Full Name'} />
              ...
              <Button btnText={'Create Account'} customClassCSS={'bg-light-green'} type={'submit'} />
            </form>
            <p className="w-[89%] font-montserrat text-xl text-black font-normal text-left mt-2">Already have an account?
              <NavLink to={'/login'}>
                <span className="text-light-green"> Log In</span>
              </NavLink>
            </p>
          </div>
        </div>
